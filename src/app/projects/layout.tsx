    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-900">
            <header className="h-14 border-b border-slate-200 bg-white/80 backdrop-blur-xl flex items-center px-6 gap-6">
                <Link href="/projects" className="flex items-center gap-2 text-sm font-bold text-slate-900">
                    <FolderKanban size={18} className="text-blue-400" />
                    ZIPPY <span className="text-slate-500 font-normal">PROJECTS</span>
                </Link>
                <nav className="flex items-center gap-4 flex-1">
                    <Link href="/projects" className="text-xs text-slate-600 hover:text-slate-900 transition-colors">
                        All Projects
                    </Link>
                </nav>
